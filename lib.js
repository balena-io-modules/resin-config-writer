...
(function() {
  var CombinedStream, Promise, Stream, cp, es, fixedLengthStream, fs, getPartitionInfo, parseMBR, tmp;
  ...
  exports.injectToFAT32 = function(path, destination, data) {
    var tmpFile;
    tmpFile = tmp.fileAsync().disposer(function(_arg) {
      var cleanup, fd, tmpPath;
      tmpPath = _arg[0], fd = _arg[1], cleanup = _arg[2];
      return cleanup();
    });
    return Promise.using(tmpFile, function(_arg) {
      var cleanup, fd, tmpPath;
      tmpPath = _arg[0], fd = _arg[1], cleanup = _arg[2];
